# Bitcoin's Proof of Work: An Analogy
...
The house, is the **proof of work** for the builder. Each house they build after another increases the trust level of their
...
to pass through the gates, it will be allowed to pass.

Where a builder builds their house is where they are searching for the proof of work for finding a block at current 
difficulty. According to Satoshi, the proof of work is the hash of the transaction block that starts with a particular 
number of zero bits. The search of this hash happens by incrementing the block's nonce and recomputing the hash every time.
The number of zero bits required represents the current difficulty to build a house for the sacred city.

Therefore, the work that a computer running Bitcoin is doing, is repeatedly computing a hash function until the hash starting with the 
required number of zero bits is found. The number of repeated computations is unpredictable, thus it presents competition 
among computers, where the fastest computer wins, they will run little iterations to find the hash. 
This expends electrical energy and computer time, this is the very value being stored in Bitcoins as well as securing the network.
The proof of this repeated computation (**mining** or work done by a miner) is the hash found to have started with the required number
of zero bits.
